...
def update_yaml(jsonData, key):
    ...
    foo_url = 'http://10.8.0.8:8042' # TODO: change (I need to receive from manager)
    name = jsonData['name']
    user = 'orthanc'
    password = 'orthanc'
    ...
    config['services']['orthanc-bar']['environment']['ORTHANC__REGISTERED_USERS'] = json.dumps(registered_users_data)

    # Peer (PACS) linked
    if jsonData['pacs']:
        peer_name = 'OrthancLOCAL' # TODO: change (I need to receive from manager)
        peer_host = jsonData['pacs']['host']
        peer_port = jsonData['pacs']['port']
        peer_user = 'orthanc'
        ...
        command = "/bin/sh -c 'pip3 install requests && pip3 install httplib2 --upgrade && python3 /home/orthanc/src/gateway-driver.py 127.0.0.1 8042 {} {} {} {}'".format(user, password, peer_name, foo_url)


        config['services']['orthanc-bar']['environment']['ORTHANC__ORTHANC_PEERS'] = json.dumps(orthanc_peer_data)

    ###
    else:
        if 'ORTHANC__ORTHANC_PEERS' in config['services']['orthanc-bar']['environment']:
            del config['services']['orthanc-bar']['environment']['ORTHANC__ORTHANC_PEERS']
        command = "/bin/sh -c 'tail -f /dev/null'"


    config['services']['wireguard']['command'] = "/bin/sh -c 'ping google.com'"
    config['services']['gateway-driver']['command'] = command
    config['services']['orthanc-bar']['environment']['ORTHANC__NAME'] = name
